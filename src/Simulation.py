...
class Simulation:
    ...
    self reproduce(self):
        ...
        for i in self.geneticSnake[self.n_snakes/2:]:
            i.brain.crossDNAAndMutate(self.geneticSnake[random.randint(
                0, self.n_snakes/2)].brain, self.geneticSnake[random.randint(0, self.n_snakes/2)].brain)

    self showBestN(self, N = 10): 
        self.field.gui = True
        self.visible = True
        self.sortSnakesForFitness()
        for i in self.geneticSnake:
            i.changeVisibility(False)
        for i in self.geneticSnake[self.n_snakes - N :]:
